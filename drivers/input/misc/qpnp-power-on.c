...
#define OPPO_PMIC_WD_DEFAULT_RST_TYPE PON_POWER_OFF_WARM_RESET
#else
...
#ifdef VENDOR_EDIT
/*xing.xing@BSP.Kernel.Driver, 2019/06/15, Add for print vol up/down or pwk*/
...
static int qpnp_poweroff(struct device *dev)
{
	...
	if(sys_reset_dev == NULL || sys_reset_dev != pon)
		return 0;
	qpnp_pon_wd_pet();
	//wen.luo@BSP.Kernel.Stability, 2019-08-27, qpnq_poweroff normal reboot keep pmicwdt enable
	//qpnp_pon_wd_config(0);
	return 0;
}
