...
build_kernel() {
	...
	make -j"$PROCS" O=out \
		...
		if [ "$PTTG" = 1 ]
 			then
				tg_post_build "error.log" "<b>❌ Build failed to compile after $((DIFF 		/ 60)) minutes </b>"
			fi
		fi
...
gen_zip() {
	...
	then
		python2 "libufdt/utils/src/mkdtboimg.py" \
					create "out/arch/arm64/boot/dtbo.img" --page_size=4096 out/arch/arm64/boot/dts/19771/sdmmagpie-idp-overlay.dtbo
